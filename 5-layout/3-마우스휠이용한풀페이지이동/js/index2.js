...
//브라우저창 크기가 바뀌면, 새로 계산한 wh높이에 따라 현재 페이지(a) 위치로 다시 스크롤
...
areas.forEach((area,index)=>{
  ...
  area.addEventListener('wheel', (e) => { //각 해당영역에서 휠 돌렸을때
...
function showSection(id) {
  ...
  tl.to(title, { y: 50, opacity: 1, duration: 1 })
  tl.to(paragraph, { y: 50, opacity: 1, duration: 1 }, "-=0.5");
}

//스크롤이벤트
window.addEventListener('scroll', function() {
  let sc = window.scrollX;  //현재 스크롤의 위치
...
window.addEventListener('keydown', (e) => {
  ...
  let prevA = a; // 이동 전 인덱스 저장

  // 위쪽 화살표
  if (e.key === 'ArrowLeft') {
    a--;
    if (a < 0) a = 0; // 0 이하로 내려가지 않게
  }

  // 아래쪽 화살표
  if (e.key === 'ArrowRight') {
    a++;
    if (a > t) a = t; // t(마지막 섹션) 이상으로 올라가지 않게
  }

  // 위치가 변하지 않았다면(즉, 이미 끝인데 또 누름) 아무 것도 하지 않음
  if (a === prevA) return;
