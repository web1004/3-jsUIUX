...
이게 가능한 이유는 e.target 을 이용하면 이벤트 리스너가 알아서 "이 카테고리를 누르는거구나"라고 알아서 실제 클릭하고 요소한테 이벤트를 실행해주기 때문이다.*/
...
tab.addEventListener('click', (e) => {
  ...
  //문제:전체(부모)에 클릭이벤트를 걸고 있어서 탭메뉴뿐만 아니라 콘텐츠영역을 클릭했을때도 click이벤트가 발생하는 것임
  //탭메뉴에는 data-alt="tab1" 같은 속성이 있지만 콘텐츠는 클릭하면 없기때문에 undefined가 됨
  //해결:li 요소만 반응하도록 제한
  if (e.target.tagName !== 'LI') return; //클릭된 요소가 <li>가 아닐 경우엔 아무 일도 하지 말고 함수를 종료
